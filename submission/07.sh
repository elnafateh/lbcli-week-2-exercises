# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
 raw_tx=""

# 1. Parse the TxID of the raw transaction
PARENT_TXID=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.txid')

# 2. Supply BOTH inputs to have enough total capacity for a 0.20 BTC output
INPUTS="[
  {\"txid\":\"$PARENT_TXID\",\"vout\":0},
  {\"txid\":\"$PARENT_TXID\",\"vout\":1}
]"

# 3. Define the destination output (20,000,000 satoshis = 0.20 BTC)
OUTPUTS="{\"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP\": 0.20}"

# 4. Create the multi-input raw transaction
NEW_RAW_TX=$(bitcoin-cli -regtest createrawtransaction "$INPUTS" "$OUTPUTS")

# 5. Output only the raw hex string
echo -n "$NEW_RAW_TX"
